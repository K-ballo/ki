...
// complex
concept complex< T >( class C )
{
    class value_type = T;
    class reference_type = value_type qualifiers_of( T ) ref;

    [self]real -> reference_type;
    [self]imaginary -> reference_type;
        
    operator +( complex operand ) -> complex
    {
    ...
    operator *( complex left, complex right ) -> complex
    {
        return complex{ left.real * right.real - left.imaginary * right.imaginary, left.imaginary * right.real + left.real * right.imaginary };
    }
    operator /( complex left, complex right ) -> complex
    {
        assert( right.real != 0 || right.imaginary != 0 );
        value_type const denominator = pow( right.real, 2 ) + pow( right.imaginary, 2 );

        return complex{ (left.real * right.real + left.imaginary * right.imaginary) / denominator, (left.imaginary * right.real - left.real * right.imaginary) / denominator };
    }
        
    abs( complex complex ) -> value_type
    {
        return square_root( pow( complex.real, 2 ) + pow( complex.imaginary, 2 ) );
    }
    square_magnitude( complex complex ) -> value_type
    {
        return pow( complex.real, 2 ) + pow( complex.imaginary, 2 );
    }
...
concept complex< T >( complex< T > c )
{
    [self]real -> T ref
    {
        return c._real;
    }
    [self]imaginary -> T ref
    {
        return c._imaginary;
    }
};

concept complex< T >( real< T > const r )
{
    [self]real -> T const ref
    {
        return r;
    }
    [self]imaginary -> T const ref
    {
...
concept imaginary< T >( class C )
{
    requires real( T );

    [self]value -> T const ref;

    operator +( imaginary operand ) -> imaginary
    {
        return imaginary{ +operand.value };
    }
    operator -( imaginary operand ) -> imaginary
    {
        return imaginary{ -operand.value };
    ...
    operator +( imaginary left, imaginary right ) -> imaginary
    {
        return imaginary{ left.value + right.value };
    }
    operator -( imaginary left, imaginary right ) -> imaginary
    {
        return imaginary{ left.value - right.value };
    ...
    abs( imaginary imaginary ) -> value_type
    {
        return value;
    }
};
...
concept complex< T >( imaginary< T > const i )
{
    [self]real -> T const ref
    {
        static T const zero = {0};
